...
dense_size = 200  # size of the 2 hidden layers in networks
...
for i in range(parallel_environments):
    os.system('gnome-terminal --tab --working-directory=WORK_DIR -- zsh -c "export '
              ...
              'turtlebot3_gazebo {}"'.format(1131 + i, 1134 + i, args.ros_environment))
    time.sleep(2)
time.sleep(args.parallel_environments * 2)
...
if __name__ == "__main__":
    ...
    learner_w_queue = mp.Queue(maxsize=parallel_environments)
    replay_priorities_queue = mp.Queue(maxsize=64)

    # Data sampler
    batch_queue = mp.Queue(maxsize=batch_queue_size)
    p = mp.Process(target=sampler_worker, args=(replay_queue, batch_queue, replay_priorities_queue, training_on,
    ...
    p = mp.Process(target=learner_worker, args=(training_on, policy_net, target_policy_net, learner_w_queue,
                   replay_priorities_queue, batch_queue, update_step, experiment_dir))
    processes.append(p)

    # Single agent for exploitation
    p = mp.Process(target=agent_worker, args=(target_policy_net, None, global_episode, 0, "exploitation",
                                              experiment_dir, training_on, replay_queue, update_step))
    processes.append(p)

    # Agents (exploration processes)
    for i in range(1, parallel_environments):
        p = mp.Process(target=agent_worker, args=(policy_net_cpu, learner_w_queue, global_episode, i, "exploration",
                                                  experiment_dir, training_on, replay_queue, update_step))
        processes.append(p)

...
class BUG2:
    def __init__(self):
        ...
        self.dist = 0.0
        self.first = True
        self.colission_distance = 0.18 * 2
    ...
    def get_action(self, state):
        ...
        if self.dist < 0.45 and (reg_values[2] > self.colission_distance and reg_values[3] > self.colission_distance and reg_values[1] > self.colission_distance):
            if self.flag == 0:
                self.angle_towards_goal(angle=state[-2])
            elif self.flag == 1:
                self.move(angle=state[-2], distance=state[-1])

        elif self.dist < 0.45 and reg_values[3] < self.colission_distance:
            self.flag_1 = 1
        ...
        elif self.dist < 0.45 and self.flag_1 == 1:
            if self.flag == 0:
                self.angle_towards_goal(angle=state[-2])
            elif self.flag == 1:
                self.move(angle=state[-2], distance=state[-1])
